...
import { decryptPhone, hashPhone } from '../utils/crypto';
  ...
  async routeIncomingSms(args: RouteIncomingSmsArgs): Promise<SmsRouteDecision> {
    ...
    for (const sid of sessionIds) {
      ...
      if (fromHash === s.party_a_hash) {
        matched = { sessionId: sid, tenantId: s.tenant_id, callerIsA: true };
        break;
      }
      if (fromHash === s.party_b_hash) {
        matched = { sessionId: sid, tenantId: s.tenant_id, callerIsA: false };
        break;
      }
    }
    ...
    try {
      ...
      await db('sms_records').insert({
        ...
        from_phone_hash: hashPhone(args.fromPhone, matched.tenantId),
        to_phone_hash: hashPhone(destination, matched.tenantId),
        body: args.body,
        status: 'PENDING',
        created_at: new Date(),
      });
